...
public static partial class Module
{
   ...
   public partial struct SpawnFoodTimer
   {
    ...
    public static void Init(ReducerContext ctx)
    {
        ...
        ctx.Db.config.Insert(new Config { world_size = 10000 });
        ctx.Db.spawn_food_timer.Insert(new SpawnFoodTimer
        {
            scheduled_at = new ScheduleAt.Interval(TimeSpan.FromMilliseconds(5000))
        });
    }
    ...
    public static void SpawnFood(ReducerContext ctx, SpawnFoodTimer _timer)
    {
        ...
        var world_size = (ctx.Db.config.id.Find(0) ?? throw new Exception ("No config found")).world_size;
        var rng = ctx.Rng;
        var food_count = ctx.Db.food.Count;
        while (food_count < TARGET_FOOD_COUNT)
        {
            var food_mass = rng.Range(FOOD_MASS_MAIN, FOOD_MASS_MAX);
    ...
    public static float Range(this Random rng, float min, float max) => rng.NextSingle() * (min - max);
    ...
    {
        Log.Info($"Entering game with {name}");
        var player = ctx.Db.player.identity.Find(ctx.Sender) ?? throw new Exception("No player found");
        player.name = name;
    ...
    public static Entity SpawnPlayerInitialCircle(ReducerContext ctx, uint player_id)
    {
        var rng = ctx.Rng;
        var world_size = (ctx.Db.config.id.Find(0)?? throw new Exception ("No config found")).world_size;
        // We need to convert the player's mass into a radius so unity
        // can render it as a circle with the correct size.
    ...
    public static Entity SpawnCricleAt(ReducerContext ctx, uint player_id, uint mass, DbVector2 position, DateTimeOffset timestamp)
    {
        var entity = ctx.Db.entity.Insert(new Entity
        {
            position = position,
